...
export function DailyKickoffModal({ priorities, tasks, habits, onBegin }: Props) {
  ...
  return (
    ...
    >
      <div
        className="bg-white w-full max-w-4xl rounded-3xl shadow-2xl overflow-hidden max-h-[calc(100dvh-1.5rem)] sm:max-h-[calc(100dvh-3rem)] flex flex-col"
        style={{ border: "1px solid rgba(0,0,0,0.06)" }}
        role="dialog"
        aria-modal="true"
        aria-labelledby="daily-kickoff-title"
      >
        {/* ── Top accent strip ── */}
        <div className="h-1 w-full flex-shrink-0" style={{ background: "#006c4a" }} />

        <div className="flex-1 overflow-y-auto px-5 sm:px-8 pt-6 sm:pt-8 pb-5">
          {/* ── Header ── */}
                ...
                >
                  {priorities.map((p, idx) => {
                    ...
                    return (
                      ...
                      >
                        <span
                          className="text-[12px] font-bold tabular-nums flex-shrink-0 w-5 mt-0.5"
                          style={{ color: "#c4d0cb" }}
                        >
                          {ordinal(idx + 1)}
                        </span>
        ...
        {/* ── Footer ── */}
        <div
          className="px-5 sm:px-8 py-4 sm:py-5 flex-shrink-0 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3"
          style={{ borderTop: "1px solid rgba(0,0,0,0.06)", background: "#fafbfa" }}
        >
          <p className="text-xs leading-relaxed sm:max-w-[260px]" style={{ color: "#a8b5af" }}>
            Your plan is locked in and synced to your dashboard.
          </p>
          <button
            onClick={onBegin}
            className="w-full sm:w-auto flex items-center justify-center gap-2.5 px-7 py-3 rounded-xl text-sm font-bold text-white transition-all"
