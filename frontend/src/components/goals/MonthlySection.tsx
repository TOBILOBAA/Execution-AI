    ...
    <div className="space-y-8">
      ...
      <section>
        ...
        <div className="space-y-3">
          {mainGoals.map((goal) => (
            <GoalCard key={goal.id} goal={goal} onEdit={onEdit} isMain />
          ))}
          <AddGoalPlaceholder label="Add Main Goal" onClick={onAdd} />
        </div>
      </section>
      ...
      <section>
        ...
        <div className="grid md:grid-cols-2 gap-4">
          ...
          <AddGoalPlaceholder label="Add Secondary Goal" onClick={onAdd} />
        </div>
      </section>
    </div>
  );
...
function GoalCard({
  ...
  return (
    <div
      className={cn(
        "bg-white rounded-xl shadow-sm overflow-hidden cursor-pointer group",
        isMain ? "border-l-4 border-[--color-primary]" : "border border-[--color-outline-variant]/10",
