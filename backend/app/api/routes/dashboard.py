...
def get_dashboard(session_id: UUID, db: Client = Depends(get_db)):
    """
    ...
    """
    return dashboard_service.get_dashboard(db, session_id)


@router.get("/{session_id}/next-day-review")
def get_next_day_review(session_id: UUID, db: Client = Depends(get_db)):
    """Return the persisted next-login review payload for today's kickoff."""
    return dashboard_service.get_next_day_review(db, session_id)
